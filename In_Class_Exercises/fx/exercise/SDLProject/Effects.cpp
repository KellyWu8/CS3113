...
Effects::Effects(glm::mat4 projection_matrix, glm::mat4 view_matrix)
{
    ...
    m_view_matrix_offset = glm::vec3(0.0f);
...
void Effects::update(float delta_time)
{
   switch (m_current_effect)
   {
       ...
       case SHAKE:
           m_timer -= delta_time * ; // time for the shaking effect left
            if (m_timer <= 0.0f) {
                m_view_matrix_offset = glm::vec3(0.0f, 0.0f, 0.0f);
                m_current_effect = NONE;
            }
            else {
                float offset = ((float)rand() / 0.5f);
                m_view_matrix_offset = glm::vec3(offset, offset, 0.0f);

                /*
                float up_offset = -1.0f;
                float down_offset = 1.0f; //((float)rand() / 10);

                if (m_timer % 2.0f == 0.0f) {
                    m_view_matrix_offset = glm::vec3(up_offset, up_offset, 0.0f);
                }
                else {
                    m_view_matrix_offset = glm::vec3(down_offset, down_offset, 0.0f);
