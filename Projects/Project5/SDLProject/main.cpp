...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Project 5: Platformer",
...
void update()
{
    ...
    g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-g_current_scene->m_state.player->get_position().x, 0.0f, 0.0f));
    ...
    // Switching scenes
    if (g_current_scene == g_levelA && g_current_scene->m_state.player->m_num_of_killed_enemies == g_current_scene->m_number_of_enemies)    // LevelA -> LevelB
    {
        switch_to_scene(g_levelB);
    }
    else if (g_current_scene == g_levelB && g_current_scene->m_state.player->m_num_of_killed_enemies == g_current_scene->m_number_of_enemies)    // LevelB -> LevelC
    {
...
void render()
{
    ...
    // Text on Menu Scene
    if (g_current_scene == g_menu) {
        Utility::draw_text(&g_program, g_text_texture_id, std::string("MINI HOPPER"), 0.5f, 0.01f, glm::vec3(-2.5f, 2.0f, 0.0f));
        Utility::draw_text(&g_program, g_text_texture_id, std::string("Press enter to start"), 0.45f, 0.01f, glm::vec3(-4.5f, 1.5f, 0.0f));
    }

    // Player has no more lives left
    if (g_curr_lives_left <= 0) {
        Utility::draw_text(&g_program, g_text_texture_id, std::string("You Lose!"), 0.5f, 0.01f, glm::vec3(-2.0f, 2.0f, 0.0f));
        g_game_is_running = false;
    }

    // Player wins the game
    if (g_current_scene == g_levelC && g_current_scene->m_state.player->m_num_of_killed_enemies == g_current_scene->m_number_of_enemies)
    {
        Utility::draw_text(&g_program, g_text_texture_id, std::string("You Win!"), 0.5f, 0.01f, glm::vec3(g_current_scene->m_state.player->get_position().x - 2.0f, 2.0f, 0.0f));
        g_game_is_running = false;
    }
